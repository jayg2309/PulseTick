...
export class GroupController {
        ...
        .map((membership) => ({
          ...
          inviteCode:
            membership.role === MemberRole.OWNER
              ? (membership.group as any).inviteCode
              : undefined,
    ...
    try {
      ...
      res.json({
        group: {
          ...
          isPublic: group.isPublic,
          inviteCode:
            membership.role === MemberRole.OWNER ? group.inviteCode : undefined,
          expiresAt: group.expiresAt,
          createdBy: group.createdBy,
          createdAt: group.createdAt,
          memberCount,
  ...
  static async updateMemberRole(
    ...
    try {
      ...
      logger.info(
        `Member role updated in group ${groupId}: ${targetUserId} -> ${role}`
      );
  ...
  static async banMember(
    ...
    try {
      ...
      logger.info(
        `Member banned from group ${groupId}: ${targetUserId} by ${userId}`
      );
  ...
  static async generateNewInviteCode(
    req: AuthenticatedRequest,
    res: Response
  ): Promise<void> {
    try {
